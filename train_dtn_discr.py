...
from fairseq.data import iterators_dtn
from fairseq.trainer_dtn_discr import Trainer
...
def main(args, init_distributed=False):
    ...
    if not load_checkpoint(args, trainer, epoch_itr):
        trainer.dummy_train_step([dummy_batch], domain=TRAIN_TAG)
...
def train(args, trainer, task, epoch_itr):
    ...
    for domain in itr.keys():
        itr_groups[domain] = iterators_dtn.GroupedIteratorDtn(itr[domain], update_freq)
        progress_len = progress_len + len(itr_groups[domain])
    progress = progress_bar.build_progress_bar(
        args, range(progress_len), epoch_itr.epoch, no_progress_bar='simple',
    )
    ...
    for i, _ in enumerate(progress, start=epoch_itr.iterations_in_epoch):
        ...
        log_output = trainer.train_step(samples, epoch=epoch_itr.epoch, domain=domain)
    ...
    for subset in subsets:
        ...
        for domain, data_valid in task.dataset(subset).items():
            ...
            for sample in progress:
                log_output, src_target_hypo_str = trainer.valid_step(sample, domain=domain)
