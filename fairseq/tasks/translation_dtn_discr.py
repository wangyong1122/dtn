...
@register_task('translation_dtn_discr')
class TranslationDtnDiscrTask(FairseqTask):
    """
    ...
    def add_args(parser):
        ...
        # classifier
        parser.add_argument('--lambda-domain', default=1, type=float,
                            help='lambda domain')
        parser.add_argument('--lambda-domain-adv', default=1, type=float,
                            help='lambda domain adv')
        parser.add_argument('--lambda-domain-adv-entropy', default=0.1, type=float,
                            help='lambda domain entropy')
    ...
    def load_pretrained_model(path, src_dict_path, tgt_dict_path, arg_overrides=None):
        ...
        task = TranslationDtnDiscrTask(args, src_dict, tgt_dict)
    ...
    def __init__(self, args, src_dict, tgt_dict):
        ...
        self.lambda_domain = self.args.lambda_domain
        self.lambda_domain_adv = self.args.lambda_domain_adv
        self.lambda_domain_adv_entropy = self.args.lambda_domain_adv_entropy
    ...
    def train_step(self, sample, model, criterion, optimizer, ignore_grad=False, domain=None, mode='nmt', num_updates=None):
        """
        ...
        self._loss_domain = model.dtn_discrs['domain'].loss
        self._loss_domain_adv = model.dtn_discrs['domain_adv'].loss
        self._loss_domain_adv_entropy = model.dtn_discrs['domain_adv'].loss_entropy

        if mode=='nmt':
            loss = loss + self.lambda_domain * self._loss_domain + \
                   self.lambda_domain_adv_entropy * self._loss_domain_adv_entropy
        elif mode=='adv':
            loss = self.lambda_domain_adv * self._loss_domain_adv
        else:
