...
class SequenceGenerator(object):
    ...
    def generate(
        ...
        **kwargs
        ...
        encoder_outs = model.forward_encoder(encoder_input, domain=domain)
...
class EnsembleModel(torch.nn.Module):
    ...
    @torch.no_grad()
    def forward_encoder(self, encoder_input, domain=None):
        if not self.has_encoder():
            return None
        return [model.dtn_blocks[domain](model.encoder(**encoder_input)) for model in self.models]
