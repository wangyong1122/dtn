...
def main(args):
    ...
    models, _model_args = utils.load_ensemble_for_inference(
        args.path.split(':'), task, model_arg_overrides=args.model_overrides,
    )
    ...
    num_sentences = 0
    has_target = True
    assert args.valid_domains==args.test_domains
    DOMAIN_TAG=args.test_domains[0]
    with progress_bar.build_progress_bar(args, itr) as t:
        ...
        for sample in t:
            ...
            gen_timer.start()
            hypos = task.inference_step(generator, models, sample, prefix_tokens, domain=DOMAIN_TAG)
    ...
    if args.decoding_path is not None:
        ...
        with open(os.path.join(args.decoding_path, 'decoding.txt'), 'w') as f:
            for sents in hyp_sents:
                ...
                for sent in sents:
                    f.write(sent+'\n')
    num_ref = args.num_ref[args.test_domains[0]]
    ref_path = []
    for i in range(int(num_ref)):
        ref_path.append(os.path.join(args.valid_decoding_path, args.test_domains[0], args.gen_subset+'.tok.'+args.target_lang+str(i)))
